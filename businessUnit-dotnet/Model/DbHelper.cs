...
namespace businessUnit.Model
{
    public class DbHelper
    {
        ...
        public businessUnitModel GetBusinessUnitsById(int id)
        {
            ...
            var row = context.businessUnits.Where(d => d.id.Equals(id)).FirstOrDefault(); // .Where is LINQ query and FirstOrDefault gives no error and gives a null value if the Id is not found 
        ...
        public void SaveOrder(businessUnitModel details)
        {
            ...
            context.SaveChanges(); // SaveChanges are used to hit the bookModel details with the Database
        }
        ...
        public void EditLocation(int id,businessUnitModel details)
        {
           ...
           edit = context.businessUnits.Where(d => d.id.Equals(id)).FirstOrDefault(); // or we can use Find(id) property instead of where () property
